...
function mimeTypeForTarget(target: string): string {
  const ext =
    target.split('?')[0].split('#')[0].split('.').pop()?.toLowerCase() ?? ''
  return IMAGE_MIME_TYPES[ext] ?? 'application/octet-stream'
}

// The built-in stylesheets loaded for each `asciidoc.preview.defaultStyle`
// value, in cascade order. A `Record` over the full union keeps this table
// exhaustive at compile time: adding a style to `AsciidocPreviewDefaultStyle`
// without adding an entry here is a type error, not a silent fallback.
const DEFAULT_STYLE_SHEETS: Record<AsciidocPreviewDefaultStyle, string[]> = {
  vscode: ['asciidoctor-editor.css', 'asciidoctor-editor-enhancements.css'],
  asciidoctor: ['asciidoctor-default.css'],
  antora: ['asciidoctor-default.css', 'asciidoctor-antora.css'],
  github: [
    'asciidoctor-default.css',
    'asciidoctor-github-colors.css',
    'asciidoctor-github.css',
  ],
}
...
/**
 * Hash (djb2, like the per-block `data-h-*` roles) the document-driven parts of
 * the webview "shell" — everything the incremental morph of `#preview-root`
 * cannot update: the `<head>` styles/scripts (MathJax, the syntax highlighter
 * and its per-language scripts, docinfo) and the `<body>` attributes. The
 * preview compares this fingerprint between renders and falls back to a full
 * webview reload when it changes, e.g. when `:stem:` or `:source-highlighter:`
 * is toggled — otherwise MathJax/highlight.js would never be (un)loaded and the
 * unchanged blocks would keep their stale rendering. The `nonce` (regenerated
 * on every render) is stripped so the fingerprint is stable across renders of
 * an identical document.
 */
function hashShellParts(parts: string[], nonce: string): string {
  const signature = parts.join('\x00').split(nonce).join('')
  // djb2
  let hash = 5381
...
function getCspForResource(
  ...
  // `Array.prototype.join` renders `undefined` entries as an empty string, so
  // an unset Kroki server URL simply contributes no extra CSP source.
  krokiServerUrl: string | undefined,
  nonce: string,
): string {
  if (
  ...
  const rules = {
    ...
    // `image::foo.svg[opts=interactive]` renders as `<object data="foo.svg">`.
    // Chromium treats an <object> that navigates to an SVG/HTML document as a
    // nested browsing context gated by `frame-src` (falling back to
    // `child-src`/`default-src` when unset), not just `object-src` — without
    // this the interactive SVG silently fails to load in the webview.
    'object-src': ["'self'", rule, 'https:', 'data:', krokiServerUrl],
    'frame-src': ["'self'", rule, 'https:', 'data:', krokiServerUrl],
  ...
  if (securityLevel === AsciidocPreviewSecurityLevel.AllowInsecureContent) {
    ...
    rules['frame-src'] = [...rules['frame-src'], 'http:']
...
export class AsciidoctorWebViewConverter {
   ...
   */
  // `node` stays `any`: convert() receives every AST node type (document,
  // inline_anchor, image, pass, …) and reads Opal-era properties (`.type`,
  // `.target`, `.text`) the typings do not declare.
  async convert(node: any, transform?: string) {
    ...
    if (nodeName === 'document') {
      ...
      const bodyCssClasses = this.getBodyCssClasses(node)
      const effectiveStyle = this.resolveEffectiveDefaultStyle(this.config)
      const mathJaxMarkup = this.generateMathJax(
        node,
        webviewResourceProvider,
        nonce,
      )
      // See hashShellParts. The stylesheet attributes are fingerprinted through
      // their resolved value (not the full getStyles output, whose image
      // stabilizer styles legitimately vary between incremental renders).
      const shellFingerprint = hashShellParts(
        [
          ...
          node.isAttribute('icons', 'font') ? 'icons-font' : '',
          effectiveStyle,
        ],
        nonce,
      )
   ...
   */
  private resolveImageUri(node: AbstractNode, target: string): vscode.Uri {
  ...
  private generateFootnotes(node: AsciidoctorDocument) {
    if (node.hasFootnotes() && !node.isAttribute('nofootnotes')) {
      const footnoteItems = node.getFootnotes().map((footnote: Footnote) => {
...
<script type="module" nonce="${nonce}">
  ...
  private generateFooter(node: AsciidoctorDocument) {
    if (node.getNofooter()) {
      return ''
    }
    const footerInfos = []
...
${footerInfos.join('\n')}
  ...
  private async getDocumentHeader(node: AsciidoctorDocument) {
    if (node.getNoheader()) {
      return ''
    }
...
${headerContent}
  ...
  private async generateArticleHeader(node: AsciidoctorDocument) {
    const content = []
    if (node.hasHeader()) {
      if (!node.getNotitle()) {
        // With `partition: true` getDoctitle always returns a DocumentTitle,
        // but the typings declare the unpartitioned union.
        const doctitle = node.getDoctitle({
          partition: true,
          sanitize: true,
        }) as DocumentTitle
        content.push(
    ...
    if (
      ...
      const outline = await (node.getConverter() as ConverterBase).convert(
        node,
        'outline',
      )
  ...
  private async generateHeaderDetails(node: AsciidoctorDocument) {
    const details = []
...
${details.join('\n')}
  ...
  private async generateManPageHeader(node: AsciidoctorDocument) {
    let tocContent = ''
    if (
      node.hasSections() &&
      node.hasAttribute('toc') &&
...
${node.hasAttribute('manpurpose') ? this.generateManNameSection(node) : ''}`
  ...
  private generateManNameSection(node: AsciidoctorDocument) {
    let mannameTitle = node.getAttribute('manname-title', 'Name')
    const nextSection = node.getSections()[0]
    const nextSectionTitle = nextSection?.getTitle()
    if (
      nextSectionTitle &&
  ...
  private getBodyCssClasses(node: AsciidoctorDocument) {
    const classes = [
      'vscode-body',
  ...
  // `defaultStyleExplicit` is false when `defaultStyle` only resolved
  // through the deprecated `useEditorStyle` boolean (true -> 'vscode',
  // false -> 'asciidoctor') rather than an explicit `preview.defaultStyle`
  // setting — neither value of that legacy boolean expresses an opinion
  // about Antora, so an Antora-active document falls back to the
  // Antora-approximating theme in both cases, per the Antora support spec.
  // An explicit `defaultStyle` choice (including an explicit `vscode`)
  // always wins.
  private resolveEffectiveDefaultStyle(
    config: AsciidocPreviewConfiguration,
  ): AsciidocPreviewDefaultStyle {
    if (!config.defaultStyleExplicit && this.antoraDocumentContext) {
      return 'antora'
    }
  ...
  private getStyles(
    ...
    config: AsciidocPreviewConfiguration,
    effectiveStyle: AsciidocPreviewDefaultStyle,
    state?: any,
  ): string {
    ...
    const replacementStyle =
      config.previewStyle !== ''
        ...
        : this.resolveDocumentStylesheet(node)
    if (replacementStyle === '') {
      for (const stylesheet of DEFAULT_STYLE_SHEETS[effectiveStyle]) {
        baseStyles.push(
          `<link rel="stylesheet" type="text/css" href="${webviewResourceProvider.asMediaWebViewSrc('media', stylesheet)}">`,
        )
      }
    }
