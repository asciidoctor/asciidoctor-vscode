  ...
  implements vscode.DocumentDropEditProvider
{
  ...
  async provideDocumentDropEdits(
    ...
    const enabled = vscode.workspace
      .getConfiguration('asciidoc', textDocument)
  ...
  uris.forEach((uri, index) => {
    ...
    if (docUri.scheme === uri.scheme && docUri.authority === uri.authority) {
      const imageRelativePath = path
        .relative(URI.Utils.dirname(docUri).fsPath, uri.fsPath)
        .replace(/\\/g, '/')
      if (imagesDirectory && imageRelativePath.startsWith(imagesDirectory)) {
        imagePath = encodeURI(
          imageRelativePath.substring(imagesDirectory.length),
        )
      } else {
    ...
    snippet.appendText(
      imageFileExtensions.has(ext) ? `image::${imagePath}[]` : '',
    )
