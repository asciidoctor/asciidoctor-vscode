  ...
  implements WorkspaceAsciidocDocumentProvider
{
  ...
  async getAllAsciidocDocuments() {
    const resources = await findFiles('**/*.adoc')
    const docs = await Promise.all(
      resources.map((doc) => this.getAsciidocDocument(doc)),
    )
  ...
  private async getAsciidocDocument(
    resource: vscode.Uri,
...
export default class AsciidocWorkspaceSymbolProvider
  implements vscode.WorkspaceSymbolProvider
{
  private _symbolCache = new Map<
    string,
    Lazy<Promise<vscode.SymbolInformation[]>>
  >()
  ...
  public async provideWorkspaceSymbols(
    ...
    if (!this._symbolCachePopulated) {
      ...
      this._workspaceAsciidocDocumentProvider.onDidChangeAsciidocDocument(
        this.onDidChangeDocument,
        this,
        this._disposables,
      )
      this._workspaceAsciidocDocumentProvider.onDidCreateAsciidocDocument(
        this.onDidChangeDocument,
        this,
        this._disposables,
      )
      this._workspaceAsciidocDocumentProvider.onDidDeleteAsciidocDocument(
    ...
    const allSymbols: vscode.SymbolInformation[] = Array.prototype.concat.apply(
      [],
      allSymbolsSets,
    )
    return allSymbols.filter(
      (symbolInformation) =>
        symbolInformation.name.toLowerCase().indexOf(query.toLowerCase()) !==
        -1,
    )
  }

  public async populateSymbolCache(): Promise<void> {
    const asciidocDocumentUris =
      await this._workspaceAsciidocDocumentProvider.getAllAsciidocDocuments()
