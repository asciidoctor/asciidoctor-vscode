...
import { parseSiteManifest, RemoteAntoraCatalog } from './siteManifest.js'
...
let remoteCatalogPromise: Promise<RemoteAntoraCatalog | undefined> | undefined
...
function getSiteManifestPathSetting(): string {
  return vscode.workspace
    .getConfiguration('asciidoc.antora', null)
    .get<string>('siteManifestPath', '')
}
 ...
 * removed, and whenever the file configured through
 * `asciidoc.antora.siteManifestPath` changes (including a change of the setting
 * itself, which re-targets the watcher at the newly configured path). Returns a
 * disposable that tears everything down.
 */
export function registerAntoraCacheInvalidation(): vscode.Disposable {
  ...
  let siteManifestWatcher = createSiteManifestWatcher(invalidate)
  const siteManifestConfigListener = vscode.workspace.onDidChangeConfiguration(
    (e) => {
      if (e.affectsConfiguration('asciidoc.antora.siteManifestPath')) {
        siteManifestWatcher?.dispose()
        siteManifestWatcher = createSiteManifestWatcher(invalidate)
        invalidate()
      }
    },
  )
  return vscode.Disposable.from(
    ...staticWatchers,
    siteManifestConfigListener,
    new vscode.Disposable(() => siteManifestWatcher?.dispose()),
  )
}
...
function getRemoteCatalog(): Promise<RemoteAntoraCatalog | undefined> {
  if (remoteCatalogPromise === undefined) {
    remoteCatalogPromise = loadRemoteCatalog().catch((err) => {
      remoteCatalogPromise = undefined
      logger.error('Antora: unable to load the site manifest', err)
      return undefined
    })
  }
...
/**
 * Load the site manifest configured through `asciidoc.antora.siteManifestPath`
 * (a workspace-relative path to a site-manifest.json file as generated by
 * Antora Atlas). Used to resolve/complete `xref:` targets that point at a
 * component published elsewhere and not part of this workspace — see
 * `AntoraContext.resolveRemoteResourceId`. Returns `undefined` (logging a
 * warning) when the setting is empty, the file cannot be found, or its content
...
async function loadRemoteCatalog(): Promise<RemoteAntoraCatalog | undefined> {
  ...
  try {
    ...
    manifest = parseSiteManifest(new TextDecoder().decode(content))
  } catch (err) {
    logger.warn(
      `Antora: unable to read the site manifest at ${uri.path}: ${err}`,
    )
    return undefined
  }
  if (manifest === undefined) {
    logger.warn(
      `Antora: the site manifest at ${uri.path} is not valid JSON or is missing "components", ignoring it`,
    )
    return undefined
  }
  logger.debug(`Antora: loaded the site manifest at ${uri.path}`)
  return new RemoteAntoraCatalog(manifest)
}
...
export async function getAntoraDocumentContext(
  ...
  try {
    const [contentCatalog, remoteCatalog] = await Promise.all([
      getContentCatalog(),
      getRemoteCatalog(),
    ])
    const antoraContext = new AntoraContext(contentCatalog, remoteCatalog)
