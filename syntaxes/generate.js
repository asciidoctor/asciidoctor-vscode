...
const delimitedCodeBlockDefinition = (
  name,
  identifiers,
  sourceScope,
  language,
...
const buildGrammar = () => {
  const text = fs.readFileSync(
    path.join(__dirname, 'asciidoc.tmLanguage.base.json'),
    { encoding: 'utf8' },
  )
  const baseGrammar = JSON.parse(text)
  const delimitedCodeBlockDefs = languages.map((language) =>
    delimitedCodeBlockDefinition(
      language.name,
  ...
  const fencedCodeBlockDefs = languages.map((language) =>
  ...
  fs.writeFileSync(
    path.join(__dirname, 'asciidoc.tmLanguage.json'),
    JSON.stringify(baseGrammar, null, 2),
  )
}
