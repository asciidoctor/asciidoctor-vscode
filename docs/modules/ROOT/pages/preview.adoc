...
* `asciidoc.preview.defaultStyle` picks the built-in stylesheet used when neither `asciidoc.preview.style` nor the document's own `stylesheet`/`stylesdir` attributes are set: `vscode` (follows the VS Code editor theme, `workbench.colorTheme`), `asciidoctor` (the plain Asciidoctor.js stylesheet), `antora` (approximates the Antora Default UI), or `github` (approximates GitHub's Markdown rendering; like `vscode`, it follows the active VS Code color theme rather than needing a separate light/dark choice).
* The default is *Automatic* (an empty value): it follows the deprecated `asciidoc.preview.useEditorStyle` setting (`vscode` unless that is set to `false`, in which case `asciidoctor`), and automatically switches to `antora` when xref:spec:antora.adoc[Antora support] is active for the document. Choosing any style explicitly — including `vscode` — always overrides this.
...
`asciidoc.preview.useEditorStyle` is deprecated in favor of `asciidoc.preview.defaultStyle`; its value is still honored as long as `defaultStyle` is left at *Automatic*.
