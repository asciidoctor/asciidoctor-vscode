...
| `asciidoc.pdf.asciidoctorPdfCommandPath` | External `asciidoctor-pdf` command to execute. It accepts a full path to the binary, for instance: `/path/to/asciidoctor-pdf`.                                                                                                    | `"asciidoctor-pdf"` |
| `asciidoc.pdf.asciidoctorPdfCommandArgs` | List of arguments, for instance: `-a pdf-themesdir=resources/themes`, `-a pdf-theme=basic`. By default, it passes the following arguments: `--quiet` and `--base-dir` with the full directory path to the AsciiDoc document.      | `[]`                |
...
| `asciidoc.pdf.wkhtmltopdfCommandArgs`    | List of arguments, for instance: `--enable-local-file-access`. By default, it passes the following arguments: `--encoding utf-8`, `--javascript-delay 1000`, `--footer-center` (if enabled) and `cover` (if it has a cover page). | `[]`                |
