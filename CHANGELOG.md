...
* Upgrade Asciidoctor.js to 4.0.1, which brings several upstream fixes over 4.0.0: `allow-uri-read` is recognised again when declared with an empty value (`:allow-uri-read:`) — 4.0.0 tested the attribute's truthiness instead of its presence, so the empty form silently disabled remote reads; the structured `source_location` is restored on reader/preprocessor log messages (feeding the Problems panel and the "Asciidoctor" output channel with the right file and line); a custom attribute redefined in the document body now resolves to the value in scope at each reference (including inside list items and table cells) instead of its final value; and the JSDoc return types of the async substitutor methods are corrected to `Promise`
...
* Document the VS Code editor defaults the extension sets for AsciiDoc files via `configurationDefaults` (`editor.wordWrap`, `editor.wordBasedSuggestions` and `editor.quickSuggestions`), why word-based and automatic suggestions are turned off (they add noise around attribute references, attribute entries and cross references, where context-aware completion already applies), and how to override them — scoped under `[asciidoc]` so the language-specific value wins, and in the *Remote*/*Workspace* settings rather than the local ones when working over Remote-SSH (#398)
