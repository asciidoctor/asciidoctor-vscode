...
* Enable the `noImplicitAny` TypeScript compiler flag (step 2 of the progressive migration to `"strict": true`). Every implicitly-`any` parameter, variable and member across `src/` and the test suite is now explicitly typed; untyped dependencies gained hand-written declarations (`asciidoctor-kroki`, `@antora/content-classifier`, `@orcid/bibtex-parse-js`) or their DefinitelyTyped packages (`@types/js-yaml`, `@types/sinon`). The code was also adapted to the extension typings shipped with `@asciidoctor/core` 4.0.3: the `[mermaid]` block processor now relies on the typed block DSL (`createBlock` and contextual `process` parameters), and a local module augmentation (`src/types/asciidoctor-core.d.ts`) restores the processor-instance registration overloads and the `PreprocessorReader` members (`getIncludeDepth`, `pushInclude`) that the runtime supports but the published typings omit. Remaining before `"strict": true`: `strictNullChecks`, then `strictPropertyInitialization`
