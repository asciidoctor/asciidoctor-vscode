...
* Fix `antora` style fidelity gaps against the real Antora Default UI's own rendering, found by comparing the preview with `antora.gitlab.io/antora-ui-default` using its own preview source (added as `examples/antora-default-ui-index.adoc`, alongside the `multirepo-ssg.svg` it illustrates). Block titles/captions were the largest gap: `asciidoctor-default.css` gives one shared selector (covering admonition, example, image, listing, literal, sidebar, quote, verse, paragraph, list and table captions, plus the header byline) both a decorative serif font and a reddish-brown color, and `antora` only neutralized the font — so every one of those titles still carried the reddish-brown, and only the subset Antora's own UI actually styles (admonition/example/image/literal/listing/open/video block titles and table captions) should read gray and Roboto; the rest (quote/verse/paragraph/dlist/qlist/hdlist titles, the header byline) now reset to plain doc-colored text, matching Antora, which does not style them at all. An admonition written as a multi-paragraph block (`====`) rendered its text ~20% larger than a one-line admonition, because the `antora` font-size only targeted the surrounding `td.content`, not a nested `<p>` inside it (which `asciidoctor-default.css`'s own `p` rule sizes independently). A sidebar block's title fell into the generic heading treatment (regular weight, left-aligned) instead of Antora's bold, centered one, and a `.verseblock`'s `<pre>`, the TOC list, and callout (`<conum>`) numbers all kept `asciidoctor-default.css`'s hardcoded Open Sans instead of picking up Roboto. Also: a quoteblock's attribution line was right-aligned instead of Antora's left; code/literal blocks used a crisp 1px border and Asciidoctor's own padding instead of Antora's much softer inset shadow and tighter padding; example and sidebar block content used Asciidoctor's looser default padding instead of Antora's tighter one; `kbd` lost the subtle "keycap" box-shadow Antora gives it; and a description list's term (`dt`) was not italicized. An ordered/unordered list's own title (`.olist > .title`/`.ulist > .title`) kept the same reddish-brown even after the italic/bold treatment was added for it, because that rule set `font-family`/`font-style`/`font-weight` but never `color`. And, same bug already fixed in the `github` style: a syntax-highlighted code block's `<pre class="highlightjs highlight">` was not matched by either `[class="highlight"]` or `[class^="highlight "]` (a two-token class list, not a `"highlight "`-prefixed string), so it kept the highlight.js theme's own background (including a dark one clashing with Antora's all-light design) instead of Antora's panel background/border treatment; both the `<pre>` selector and the nested `<code class="hljs">`'s own background (set directly by the theme, painting over the `<pre>`) are now covered; that same `<code class="hljs">` also carries its own `padding` (`pre code.hljs` in the highlight.js theme), which stacked on top of the `<pre>`'s padding and made a highlighted block noticeably roomier than a plain one, so it is now zeroed out. Two of the eight gray/Roboto captions still kept the reddish-brown regardless: `antora`'s simplified `.admonitionblock .title`/`table.tableblock caption` selectors are *less* specific than `asciidoctor-default.css`'s own `.admonitionblock td.content > .title`/`table.tableblock > .title` (an extra `td.content` class, and a class beats an element in specificity), so the older, more specific rule kept winning regardless of source order; both now match `asciidoctor-default.css`'s selectors exactly, so specificity ties and the later `antora` rule wins as intended
...
* Fix `asciidoc.extensions.registerWorkspaceExtensions` in 4.0.0, where a trusted workspace's `.asciidoctor/lib/**/*.js` Asciidoctor.js extensions failed to load with `TypeError: Cannot convert undefined or null to object` (#1179). Since 4.0.0 the extension is bundled as native ESM, which has no ambient `require`; the workspace-extension loader still called the bare `require`/`require.cache`, which esbuild's bundler rewrites into a shim that falls back to a `Proxy` over a throwing stub in that context, so the error was thrown before the workspace file was ever loaded — misleadingly naming the extension file as the culprit. The loader now uses `createRequire(import.meta.url)` (from `node:module`) to get a real `require`, restoring both loading and the existing hot-reload-on-edit behaviour (each file's `require.cache` entry is still cleared before reloading)
...
* Enable `strictNullChecks` and `strictPropertyInitialization`, then collapse the flag list to `"strict": true` (final step of the progressive TypeScript strict migration). Every `null`/`undefined` gap surfaced across `src/` and the test suite is now handled explicitly (guards, fallbacks, or a documented non-null assertion where the runtime invariant already holds). Two latent bugs found in the process are fixed: `getDefaultWorkspaceFolderUri()` crashed instead of returning `undefined` when no workspace folder was open, and `BuiltinDocumentAttributeProvider.provideCompletionItems()` returned `undefined` instead of an empty array for a non-matching line (both are now handled the same way as the rest of the codebase already expected)
...
* Update dependencies, notably the runtime ones bundled into the extension: `@asciidoctor/core` (the rendering engine) from 4.0.4 to 4.0.6, `mermaid` from 11.15.0 to 11.16.0 (plus `@mermaid-js/layout-elk`), and `mathjax`/`@mathjax/mathjax-mhchem-font-extension`/`@mathjax/mathjax-newcm-font` from 4.1.2 to 4.1.3. Also `js-yaml` (used to parse `antora.yml`) from v4 to v5, whose removal of the default export is reflected in `antoraDocument.ts`'s `import { load } from 'js-yaml'`, and the dev-only `@biomejs/biome`, `typescript`, `sinon`, `@playwright/test`, `@vscode/vsce` and `@vscode/test-electron`
